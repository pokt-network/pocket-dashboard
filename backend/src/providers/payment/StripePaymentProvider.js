...
class StripePaymentProvider extends BasePaymentProvider {
  constructor(paymentProviderConfiguration) {
    ...
    /** {Stripe} */
    this._stripeAPIClient = new Stripe(
      paymentProviderConfiguration.client_secret,
      paymentProviderConfiguration.options
    );
  ...
  async createPaymentIntent(
    ...
    const paymentResultData = await this._stripeAPIClient.paymentIntents.create(
      paymentData
    );
    ...
    return new PaymentResult(
      paymentResultData.id,
      date,
      paymentResultData.client_secret,
      paymentResultData.currency,
      resultAmount,
      providerType.stripe
    );
  }
  ...
  async retrieveCardPaymentMethod(paymentMethodID) {
    const paymentMethodData = await this._stripeAPIClient.paymentMethods.retrieve(
      paymentMethodID
    );
  ...
  async retrieveCardPaymentMethods(paymentMethodIDs) {
    const cardPaymentMethods = await paymentMethodIDs.map(
      this.retrieveCardPaymentMethod
    );
  ...
  async getCustomerCardPaymentMethods(customerID) {
    /** @type {{data:*[]}} */
    const request = await this._stripeAPIClient.paymentMethods.list({
      customer: customerID,
      type: "card",
    });
