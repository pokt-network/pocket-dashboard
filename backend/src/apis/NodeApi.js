...
router.post(
  ...
  apiAsyncWrapper(async (req, res) => {
    ...
    const node = await nodeService.createNodeAccount(
      data.nodeID,
      data.nodeData
    );
    ...
    await EmailService.to(node.contactEmail).sendCreateOrImportNodeEmail(
      emailAction,
      node.contactEmail,
      nodeEmailData
    );
...
router.put(
  ...
  apiAsyncWrapper(async (req, res) => {
    ...
    if (
      await nodeService.verifyNodeBelongsToClient(
        params.nodeAccountAddress,
        req.headers.authorization
      )
    ) {
      const updated = await nodeService.updateNode(
        params.nodeAccountAddress,
        data
      );
      ...
      res
        .status(400)
        .send("Node doesn't belong to the provided client account.");
    }
  })
);
...
router.post(
  ...
  apiAsyncWrapper(async (req, res) => {
    ...
    if (
      await nodeService.verifyNodeBelongsToClient(
        data.nodeAccountAddress,
        req.headers.authorization
      )
    ) {
      const node = await nodeService.deleteNode(
        data.nodeAccountAddress,
        bodyData.user
      );
      ...
      if (node) {
        ...
        await EmailService.to(node.contactEmail).sendNodeDeletedEmail(
          node.contactEmail,
          nodeEmailData
        );
      }
...
router.post(
  ...
  apiAsyncWrapper(async (req, res) => {
    ...
    if (
      await nodeService.verifySessionForClient(
        req.headers.authorization,
        data.user
      )
    ) {
      ...
      res
        .status(400)
        .send("Node list doesn't belong to the provided client account.");
    }
  })
);
...
router.post(
  ...
  apiAsyncWrapper(async (req, res) => {
    ...
    const data = req.body;
    const { nodeUnstakeTransaction, nodeLink } = data;

    // Submit Unstake transaction to the pocket network
    await nodeService.unstakeNode(
