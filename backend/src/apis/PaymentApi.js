...
router.post(
  ...
  apiAsyncWrapper(async (req, res) => {
    ...
    if (data && userEmail && userEmail.toString() === data.user.toString()) {
      const paymentIntent = await paymentService.createPocketPaymentIntentForApps(
        data
      );
      ...
      res
        .status(400)
        .send("New intent of payment doesn't belong to the client.");
    }
  })
);
...
router.put(
  ...
  apiAsyncWrapper(async (req, res) => {
    ...
    if (
      await paymentService.verifyPaymentBelongsToClient(
        data.paymentId,
        req.headers.authorization
      )
    ) {
      const result = await paymentService.updatePaymentWithPrintableData(
        data.paymentId,
        data.userEmail,
        data.printableData
      );
...
router.post(
  ...
  apiAsyncWrapper(async (req, res) => {
    ...
    if (data && userEmail && userEmail.toString() === data.user.toString()) {
      const paymentIntent = await paymentService.createPocketPaymentIntentForNodes(
        data
      );
